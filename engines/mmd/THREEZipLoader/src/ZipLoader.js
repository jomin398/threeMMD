...
class ZipLoader {
	 ...
	 */
	async #makeblobURLs(url, zip, bypassed) {
		var fileMap = {};
		var pendings = [];
		...
		if (!bypassed) {
			baseUrl = 'blob:' + THREE.LoaderUtils.extractUrlBase(url);
		} else {
			let p = location.href;
			baseUrl = p.substring(0, p.lastIndexOf('/') + 1)
		}
	...
	load(url, onProgress, onError, bypassZipLoad) {
		...
		if (!bypassZipLoad) {
			...
			let zip = url;
			return this.#makeblobURLs(url, zip, bypassZipLoad)
				.then((fileMap) => ({
					getFileList: fileMap,
					urlResolver: function (url) {
						url = url.replace(/\\/g, '/')
