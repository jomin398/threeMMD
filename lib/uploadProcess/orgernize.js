...
import { audioContainer, spineModel, mmdModel, bgiContainer } from '../modelClass/index.js';
...
const detectMMD = str => ['vmd', 'pmx', 'pmd'].some(ext => str.includes(ext));
...
export async function orgernize(d) {
    ...
    const colect = async (obj, isBackGround) => {
        ...
        const fListHandler = async (file, zip, colect) => {
            ...
            Object.entries(files).map(([key]) => {
                if (detectSpine(key)) {
                    isSpine = true;
                } else if (detectMMD(key)) {
                    isMMD = true;
                }
                if (!isMMD && detectAudio(key)) isAudio = true;
            });
            //process
            if (!isBackGround) {
                if (hasspine && isSpine) await new Classify().spine(files, colect);
                if (isMMD) await new Classify().mmd(files, zip, file.name, colect);
                if (isAudio) await new Classify().audio(files, zip, file.name, colect);
            } else {
                if (hasspine) {
                    await new Classify().spine(files, colect)
                } else if (isMMD) {
                    await new Classify().mmd(files, zip, file.name, colect)
                } else if (isAudio) {
    ...
    if (iniFile) {
        ...
        let result = [];
        let dc = Object.values(c);
        let db = Object.values(b);
        ////console.log(db)
        // c와 b 안의 모든 요소가 모두 mmdModel 인스턴스인지 확인
        if (dc.every(item => item instanceof mmdModel)) {
            //console.log('mmd model organizing initialized')
            // if (dc.length == 0) {
            //     let charObj = new mmdModel('SchoolUniform Miku');
            //     charObj.chrFile = "https://rawcdn.githack.com/jomin398/mmd_bk/c01152b8c568ba02207c977ef6210546830e0ee7/model/miku/School%20Uniform%20Miku.zip";
            //     dc.push(charObj)
            // }
            // bgm obj 찾기
            let bgmObj = db.find(obj => obj.chrFile == null && Object.keys(obj.audios).length > 0);
            let motionObj = db.find(obj => obj.modelMotion);
            let camObj = db.find(obj => obj.chrFile == null && obj.cameraMotion);
            // 스테이지 obj 찾기

            // let stageObjs = db.find(obj => obj.chrFile != null);
            // //console.log('stage:', db, stageObjs)
            // 캐릭터 obj와 bgm obj 합치기

            // dc의 아이템 개수가 1개 이상일 때 array로 전환
            // dc에서 첫 번째 아이템만 남기고 나머지는 삭제 처리.
            if (dc.length >= 1) {
                dc[0].chrFile = dc.map(charObj => charObj.chrFile);
                dc[0].name = dc.map(charObj => charObj.name);
                dc.splice(1);
            }
            //console.log(dc[0].chrFile)
            dc[0].chrFile = chrPositioning(dc[0].chrFile);
            // 캐릭터 obj와 bgm obj 합치기
            //console.log(dc)
            dc.forEach(charObj => {
                if (charObj.chrFile !== null) {
                    //charObj.stagefile ??= stageObjs ? stageObjs.chrFile : null;
                    charObj.modelMotion = motionObj ? motionObj.modelMotion ?? null : null;
                    charObj.cameraMotion = camObj ? camObj.cameraMotion ?? null : null;
                    charObj.audios = bgmObj ? bgmObj.audios ?? null : null;
                    result.push(charObj);
                }
            })
        } else {
